...
def UpdateMovies(libraryFolderMovies):
	...
	url = "https://raw.githubusercontent.com/kodianonymous1/Anonymous/master/all_data.json"
	try:
	...
	for movie in data:
		...
		if int(movie['year'])>=int(Addon.getSetting("year_s")) and int(movie['year'])<=int(Addon.getSetting("year_e")) and x<int(Addon.getSetting("limitMovies")):
		 ...
		 if exist_folder==0:
			...
			f = open(StrFile, 'w')
			f.write("plugin://plugin.video.anonymous.wall/?wall=movies&"+urllib.urlencode(Str))
			f.close()
...
def UpdateTV(libraryFolderTV):
	# Get TV Info
	q = GetTVSearchQuery()
	url = "https://raw.githubusercontent.com/kodianonymous1/Anonymous/master/all_data_series.json"
	try:
		response = urllib.urlopen(url)
		data = json.loads(response.read())
			...
			if season[i]>0:
				...
				for x in range(1,(int(season[i])+1)):
					...
					if not os.path.exists(StrFile):
						...
						f = open(StrFile, 'w')
						f.write("plugin://plugin.video.anonymous.wall/?wall=tv&"+urllib.urlencode(encoded_dict(Str)))
						f.close()
						try:
...
def removeDisallowedFilenameChars(s):
    filename = re.sub(r'[\\/*?:"<>|]',"",s)
    return filename
	...
	if array in remove_data and remove_data[array]:
		...
		for show in remove2_data:
			...
			if not ff in FoldersRemove:
				if "plugin.video.anonymous.wall" in show['file']:
...
def main():
	...
	LOG.append("Anonymous Media Wall Ver: {0} Syn.Time: {1}".format(AddonVersion,Addon.getSetting("StartDelay")))
	...
	# Dump Log
	logfilep = xbmc.translatePath(os.path.join("special://userdata/addon_data/plugin.video.anonymous.wall/", "thewiz.wall.log")).decode("utf-8")
	logfile = open(logfilep, 'w')
	for line in LOG:
		logfile.write("\n"+line)
	logfile.close

	xbmc.executebuiltin('Notification({0}, Updated, {1}, {2})'.format(AddonName, 5000,os.path.join( AddonPath ,"icon.png" )))
