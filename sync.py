...
def UpdateMovies(libraryFolderMovies):
	...
	for movie in data:
		...
		if movie['year']>=Addon.getSetting("year_s") and movie['year']<=Addon.getSetting("year_e") and x<Addon.getSetting("limitMovies"):
		 x=x+1
...
def UpdateTV(libraryFolderTV):
	# Get TV Info
	q = GetTVSearchQuery()
	url = "http://wall.ghost-rider.ml/?wall=tv"+q
	try:
		response = urllib.urlopen(url)
		data = json.loads(response.read())
	...
	for tv in data:
		...
		if not os.path.exists(TVFolder):
			LOG.append("Wall.Sync: Create TV {0}".format(tv_name.decode("utf-8")))
			...
			if season[i]>0:
				...
				for x in range(1,(int(season[i])+1)):
					...
					if not os.path.exists(StrFile):
						...
						Str = {'imdb':tv['imdb'],'tmdb':tv['tmdb'],'trakt':tv['trakt'],'year':tv['year'],'title':tv['title'],'slug':slug,'url':"http://www.imdb.com/title/"+tv['imdb']+"/",'episode':x,"season":i,"show":tv_name,"name":tv['title']+' S%02d'%int(i)+'E%02d'%int(x),"tvdb":tv['tvdb']}
						f = open(StrFile, 'w')
						f.write("plugin://plugin.video.ghostrider.wall/?wall=tv&"+urllib.urlencode(Str))
						f.close()
						try:
